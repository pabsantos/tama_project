...
Previous studies have also investigated the vulnerability of transport and road networks using graph-based approaches and the analysis of flood occurrence. To \textcite{sharifiResilientUrbanForms2019}, road design and network topology are the two main factors that influence the resilience of road networks. Focusing on topological properties, \textcite{morelliMeasuringUrbanRoad2021} focused on vulnerability in urban road systems, highlighting methodological aspects to evaluate structural robustness under different flood scenarios. More recently, \textcite{santosVulnerabilityAnalysisComplex2023} applied complex network theory to analyze vulnerability to flooding in rural road networks in the state of Santa Catarina, Brazil, offering insights into large-scale network behavior and its implications for regional transport planning.
...
The area investigated included five parts of the Tamanduateí (TTI) river basin inside the city of São Paulo, which are the following: \emph{Ribeirão do Oratório}, \emph{Montante do Ribeirão do Oratório}, \emph{Córrego Ourives/Ribeirão dos Couros}, \emph{Ribeirão dos Meninos}, and \emph{Área de Contribuição Direta de Escoamento Difuso - Meninos/Tamanduateí} \cite{prefeituradesaopauloPlanoDiretorDrenagem2024}.

To select the road network, a more appropriate area division is the origin/destination (OD) zones from the city of São Paulo. These OD zones were developed to count and manage traffic in the city \cite{metrospPesquisaOrigemDestino2023}. Unlike the TTI section limits, OD zones are ideal for selecting the road network, avoiding the cut or separation of street blocks when establishing the scenario. All OD zones that intersected the previously selected TTI sections were considered in this work. In total, 92 OD zones were considered (from a total of 527). The map in Figure \ref{fig:area} shows the considered TTI sections and OD zones.

\begin{figure}[H]
\centering
\includegraphics[width=.7\textwidth]{../plot/study_area_map.png}
\caption{Study scenario}
\label{fig:area}
\end{figure}

From the selected OD zones, it was possible to load the road network graph using the \verb|osmnx| Python package \cite{boeingModelingAnalyzingUrban2025}. The graph object considered the network from November 2025. The flood occurrence locations were loaded as point objects from \cite{cge-spAlagamentos2025}, between 2022-01 and 2025-04. Figure \ref{fig:flood} presents the loaded road network and flood locations.

\begin{figure}[H]
\centering
\includegraphics[width=.7\textwidth]{../plot/network_flood_map.png}
...
The road network presented in this work is represented by a graph, where each intersection is a node and each road segment is an edge. Graphs are structures used to model the relations between objects \cite{barabasiNetworkScience2016}. A graph $G$ can be defined as:
...
where $V$ is the set of nodes, $\sigma(s, t)$ is the number of shortest $(s, t)$-paths, and $\sigma(s, t | e)$ is the number of those paths passing through edge $e$ \cite{brandesFasterAlgorithmBetweenness2001}.
...
The population data was collected from the Brazilian Demographic Census of 2022 \cite{ibgeCensoDemografico20222022}. The available data is aggregated by census tracts, and only the ones inside the study area were selected, by intersection to the OD zones.
...
\begin{figure}[H]
\centering
\includegraphics[width=.8\textwidth]{../plot/degree_distribution.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=.8\textwidth]{../plot/network_ebc_map.png}
...
The 7,329 census tracts loaded and selected are presented in Figure \ref{fig:popcensus}, showing the population value of each area unit. The map shows some urban voids in the study area and some high-density areas, with population values above 2,000. In total, the area has a population of 2,803,723 inhabitants.
...
The map in Figure \ref{fig:popedge} presents the estimated population for all 48,769 graph edges, with a total population of 2,137,552. Approximately 95\% of the edges have population values below 100.

\begin{figure}[H]
\centering
\includegraphics[width=.8\textwidth]{../plot/edges_population_map.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=.8\textwidth]{../plot/ebc_flood_scatter.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=.8\textwidth]{../plot/ebc_population_scatter.png}
