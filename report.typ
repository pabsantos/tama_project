...
// What everyone already knows about the topic: Floods, urban road networks, resilience, etc.
...
The area investigated included parts of the Tamanduateí (TTI) river basin inside the city of São Paulo, which are the following sections: _Ribeirão do Oratório_, _Montante do Ribeirão do Oratório_, _Córrego Ourives/Ribeirão dos Couros_, _Ribeirão dos Meninos_, and _Área de Contribuição Direta de Escoamento Difuso - Meninos/Tamanduateí_ (CITATION).

// Inside this area, 5 stations of data collection (SDC) were considered when collecting rainfall and river level data, with the following IDs: 275, 283, 413 563 and 629 (CITATION). The time period considered in this work ranged from 2022-01 to 2025-04, considering the available data from the respective sources.
...
// Three graph characterization measures were calculated. Two for nodes - degree ($k_i$) and centrality ($c_i$) - and one for edges, the edge betweenness centrality ($?$). 

// The degree of a node $i$, hence $k_i$, is the number of edges connected to that node. This is known as adjacency ($a_(i j)$). When two nodes $i$ and $j$ are said to be neighbors, then $a_(i j) != 0$. The following @eq-ki shows the computation of $k_i$ *(Costa et al. 2007)*:
...
//nFinally, edges with EBC values above the median and with flood count above the median were highlighted in a map, to check which road sections are more vulnerable to flooding. 

== Population data

// Data gathering (aopdata), H3 res 9, spatial interpolation to network edges
...
// The loaded road network graph has 21,045 nodes and 48,769 edges. 

// @fig-power shows the scatter plot of the degree ($k_i$) and clustering coefficient ($c_i$) of the road network. It is possible to observe that higher values of $k_i$ are followed by lower values of $c_i$, implying that the graph is not purely random.
